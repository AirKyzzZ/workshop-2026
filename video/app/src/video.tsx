  ...
  <AbsoluteFill style={{ backgroundColor: couleurs.fond }}>
    <Series>
      {liste.map((scene) => {
        ...
        return (
          <Series.Sequence key={scene.id} name={`Scène ${scene.id}`} durationInFrames={framesScene(scene, replique, FPS)}>
            {Contenu ? <Contenu /> : <CartonProvisoire id={scene.id} texte={replique?.texte} />}
            {replique?.genere ? <Audio src={staticFile(replique.fichier)} /> : null}
          </Series.Sequence>
        );
      })}
    </Series>
