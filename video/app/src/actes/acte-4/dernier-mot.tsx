...
const CLES = [
  { frame: 14, x: 980, y: 900 },
  { frame: 50, x: 1420, y: 470 },
  { frame: 78, x: 1392, y: 488 },
  { frame: 104, x: 1430, y: 456 },
  { frame: 114, x: 1404, y: 474 },
  { frame: 134, x: 560, y: 450 },
];
...
export const DernierMot: React.FC = () => {
  ...
  const appui = interpolate(frame, [CLIC, CLIC + 8], [0, 1], bloque);
  const choisi = avance(frame, VALIDE, 10);
  const survolDroite = dans(curseur.x, curseur.y, DROITE) && frame < CLIC ? 1 : 0;
  const survolGauche = dans(curseur.x, curseur.y, GAUCHE) ? 1 : 0;
  const resultat = avance(frame, VALIDE + 4, 12);

  return (
    <FondScene>
      <BarreCommandement p={1} />
      <Option x={GAUCHE.x} largeur={GAUCHE.largeur} p={avance(frame, PANNEAUX, 12)} survol={survolGauche} choisi={choisi} ecarte={0} titre="AFFECTER LE REMPLAÇANT">
      ...
      <Option x={DROITE.x} largeur={DROITE.largeur} p={avance(frame, PANNEAUX + 4, 12)} survol={survolDroite} choisi={0} ecarte={choisi} titre="DÉROGER">
        <div style={{ display: "flex", gap: 20 }}>
          {GARANTIES.map((g, i) => {
            const allume = avance(frame, g.a, 8);
            return (
              <div
                key={g.mot}
                style={{
